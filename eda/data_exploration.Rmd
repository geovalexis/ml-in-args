...
# Count number of SNPs per gene and sample
snps_data %>%
  group_by(sample_name, gene_name) %>%
  summarise(gene_pos_count = n_distinct(gene_pos)) %>%
  arrange(desc(gene_pos_count))

# Drop those genes with anormal number of SNPs
snps_data <- snps_data %>%
  filter(!gene_name %in% c("sspH2", "STM2240", "STM1860"))

# Boxplot with number of SNPs per gene
snps_data %>%
  group_by(sample_name, gene_name) %>%
  summarise(gene_pos_count = n_distinct(gene_pos)) %>%
  arrange(desc(gene_pos_count)) %>%
  slice_head(n = 10) %>%
  ggplot(aes(x = "", y = gene_pos_count, fill = gene_name)) +
  geom_boxplot() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  labs(x = "", y = "Number of SNPs", title = "Distribution of SNPs per gene") +
  theme_bw()
filtered_out_rows <- original_size - nrow(snps_data)
```

After filtering out SNPs with no gene name and SNPs which count for a specified number of time in the same gene, we have filtered out `r filtered_out_rows` rows.
...
# Create dataframe with remaining samples, taking into account the following:
# * it must have the same columns as snps_data_wide
# * the assigned values will depend upon the gene/pos columns. If gene is present in the sample, then the value will be NO-SNP, otherwise it will be NULL
# * Whether the gene belongs to the samples can be figured out by looking at the reference genomes
remaining_samples <- setdiff(samples_metadata$assembly_accession, snps_data_wide$sample_name)
remaining_samples_snps <- list()
for (sample in remaining_samples) {
  remaining_samples_snps[[sample]] <- refs_genomes[[as.character(samples_metadata[samples_metadata$assembly_accession == sample, "tax_id"])]]
}
